...
# öğrenmesi modeli geliştirilmesi
# istenmektedir. Modeli geliştirmeden önce gerekli olan veri analizi ve özellik mühendisliği adımlarını gerçekleştirmeniz
# beklenmektedir.
...
# Hedef değişken "outcome" olarak belirtilmiş olup; 1 diyabet test sonucunun pozitif oluşunu, 0 ise negatif oluşunu
# belirtmektedir.
...
# GÖREV 2: FEATURE ENGINEERING
           ...
           # değişkenlerde 0 değeri içeren gözlem birimleri eksik değeri ifade ediyor olabilir. Örneğin; bir kişinin glikoz
            # veya insulin değeri
           # 0 olamayacaktır. Bu durumu dikkate alarak sıfır değerlerini ilgili değerlerde NaN olarak atama yapıp
# sonrasında eksik değerlere
...
df = pd.read_csv(r"C:\Users\Derya\PycharmProjects\Diabets_ML_Project\diabetes.csv")
df.head()
...
def target_summary_with_num(dataframe, target, numerical_col):
    ...
    print(temp_df)
    # temp_df.plot(kind="pie", y=numerical_col, autopct="%1.1f%%")
    temp_df.plot(kind="bar", y=numerical_col)
    plt.show(block=True)
...
# df["NEW_AGE_CAT"].head() + "_" + df["NEW_BMI"].head().astype(str)
